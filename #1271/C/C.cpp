...
int main() {
	...
	int n ;
 	pair<int,int> shop;
 	cin >> n >> shop.first >> shop.second ; 

 	vector<int> number(4,0);
 	vector<pair<int,int>> positions(4);

 	positions[0] = {shop.first, shop.second+1};
 	positions[1] = {shop.first, shop.second-1};
 	positions[2] = {shop.first+1, shop.second};
 	positions[3] = {shop.first-1, shop.second};
 	...
 	for(int i = 0 ; i < n ; ++i){
 		...
 		for(int j = 0 ; j < 4 ; ++j){
 			if(distance(student,shop) >= distance(student,positions[j])){
 				number[j]++;
 			}
 		}
 	}
 	int maxxpos = max_element(all(number)) - number.begin();
 	cout << number[maxxpos] << endl;
 	cout << positions[maxxpos].first << " " << positions[maxxpos].second <<endl;
