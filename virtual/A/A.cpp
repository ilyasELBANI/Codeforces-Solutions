...
int main() {
	...
	while (q--) {
		...
		int minn = *min_element(all(vect));
		int maxx = *max_element(all(vect));
		...
		if (maxx - minn <= 2 ) {
			...
			if (vect[1] > vect[0] && vect[1] > vect[2] ) {
				vect[1] --;
			}
			if (vect[1] < vect[0] && vect[1] < vect[2] ) {
				vect[1] ++;
			...
			int ans = abs(vect[0] - vect[1]);
			ans += abs( vect[0] - vect[2] );
			ans += abs( vect[1] - vect[2] );
