...
int main() {
	...
	for (int i = 0 ; i < n ; ++i) {
		...
		int minn = *min_element(all(vect[i]));
		int maxx = *max_element(all(vect[i]));

		minarr.push_back(minn);
	...
	ll sol = 0 ;
	for (int i = 0 ; i < sz(minarr) ; ++i) {
		auto itr = upper_bound(all(maxarr), minarr[i]);
		// debug(minarr[i]) ;debug(*itr); debugg(distance(itr,maxarr.end()));
		sol += distance(itr,maxarr.end());
	}
	sol += (cntOk*1LL * n)  ; 
	sol += (cntOk * 1LL * (n-cntOk))  ; 
