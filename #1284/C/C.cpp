...
template<typename T>   T add(T x, T y, ll mod) { return ((x % mod)  + (y % mod)) % mod;}
template<typename T>   T mul(T x, T y, ll mod) { return ((x % mod) * (long long)(y % mod)) % mod;}
...
int main() {
	...
	for (ll i = 0; i < n; i++)
	{
		ans +=   mul(mul((n - i), dp[n - i], m) , dp[i + 1], m);
		ans %= m;
	} 
	cout << ans << endl;
