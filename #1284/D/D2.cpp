...
int main() {
	...
	deque<pair<int, pair<int, int>>> a(n) ;
	deque<pair<int, pair<int, int>>> b(n) ;

	for (int i = 0 ; i < n ; ++i) {
		cin >> a[i].first >> a[i].second.first >> b[i].first >> b[i].second.first ;
		a[i].second.second = b[i].second.second = i  ;
	}
	...
	vector<int> b_sol(n);
	for (int i = 0 ; i < n ; ++i) {
		int lo = i + 1, hi = n , mid, ans;
		// debug(lo); debugg(hi);
		while (lo <= hi) {
			mid = (lo + hi) >> 1;
			// debugg(mid);
			if ( a[mid - 1].first <= a[i].second.first) {
				ans = mid;
				lo = mid + 1;
			} else {
				hi = mid - 1;
	...
	for (int i = 0 ; i < n ; ++i) {
		...
		while (lo <= hi) {
			...
			if ( b[mid - 1].first <= b[i].second.first) {
				ans = mid;
				lo = mid + 1;
			} else {
				hi = mid - 1;
