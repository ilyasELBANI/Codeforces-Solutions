...
int main() {
	...
	while (t--) {
		...
		map<int, int> watchedShows;

		for (int ptr = 0 ; ptr < d ; ++ptr) {
			watchedShows[vect[ptr]]++;
		}
		...
		int minn = watchedShows.size();

		while (end != (int)vect.size()) {
			watchedShows[vect[st]]--;
			if (watchedShows[vect[st]] == 0 ) {
				watchedShows.erase(vect[st]);
			}
			st++;
			watchedShows[vect[end++]]++;
			minn = min(minn, (int)watchedShows.size());
		}
