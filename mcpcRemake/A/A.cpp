...
int main() {
	...
	while (t--) {
		...
		*/
		...
		long double r_sqr = (X1 * X1 + Y1 * Y1 + Z1 * Z1);
		long double dist_sqr = ((X2 - X1) * (X2 - X1) + (Y2 - Y1) * (Y2 - Y1) + (Z2 - Z1) * (Z2 - Z1)) ;

		long double angle = acos( (dist_sqr + r_sqr - r_sqr ) / (2 * sqrt(dist_sqr) * sqrt(r_sqr) ));
		debugg(angle2);
